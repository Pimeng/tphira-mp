...
说明：
- 推送带有房间上下文的实时日志事件，包括玩家加入/离开、房内聊天、准备、选曲、开始/结束对局、房主变更等
- 推送 `INFO`、`MARK`、`WARN`、`ERROR` 级别的房间事件，不受文件日志 `LOG_LEVEL` 过滤影响
- 只推送与订阅房间相关的日志
- 日志消息为服务器端格式化后的文本
...
4. 房间状态变化时会自动推送更新
5. 带房间上下文的实时日志事件会推送给所有订阅该房间的客户端（包括玩家加入/离开、房内聊天、准备、房主变更等）
