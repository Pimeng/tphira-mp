...
const loggerTestBaseDir = join(tmpdir(), "phira-mp-nodejs-logger-tests");

function makeTempDir(name: string): string {
  const dir = join(loggerTestBaseDir, name);
...
describe("Logger", () => {
  ...
  afterAll(() => {
    rmSync(loggerTestBaseDir, { recursive: true, force: true });
  });
